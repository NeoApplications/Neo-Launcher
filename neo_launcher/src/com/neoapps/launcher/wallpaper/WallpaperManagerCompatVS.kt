...
import com.neoapps.launcher.wallpaper.WallpaperColorsCompat.Companion.HINT_SUPPORTS_DARK_TEXT
...
internal class WallpaperManagerCompatVS(context: Context) : WallpaperManagerCompat(context) {
    ...
    private fun update(wallpaperColors: WallpaperColors?) {
        ...
        val platformHints = wallpaperManager
            ...
            ?.colorHints ?: 0
        var hints = 0
        if ((platformHints and HINT_SUPPORTS_DARK_TEXT) != 0) {
            hints = hints or HINT_SUPPORTS_DARK_TEXT
        }
        if ((platformHints and HINT_SUPPORTS_DARK_THEME) != 0) {
