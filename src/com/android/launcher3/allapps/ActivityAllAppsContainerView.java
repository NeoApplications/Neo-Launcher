...
public class ActivityAllAppsContainerView<T extends Context & ActivityContext>
    ...
    protected List<ActivityAllAppsContainerView<?>.AdapterHolder> mAH;
    ...
    public ActivityAllAppsContainerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        prefs = NeoPrefs.getInstance(mActivityContext);
    ...
    protected void onFinishInflate() {
        ...
        for (ActivityAllAppsContainerView<?>.AdapterHolder holder : mAH) {
            if (holder.isSearch()) {
                holder.setup(mSearchRecyclerView, /* Filter out A-Z apps */ itemInfo -> false);
            }
        }
    ...
    private void animateToSearchState(boolean goingToSearch, long durationMs) {
        ...
        if (goingToSearch) {
            ...
            mWorkManager.onActivePageChanged(getSearchHolder());
    ...
    protected void rebindAdapters(boolean mShowTabs, boolean force) {
    ...
    void setupHeader() {
        ...
        ActivityAllAppsContainerView<?>.AdapterHolder searchHolder = null;
        for (ActivityAllAppsContainerView<?>.AdapterHolder holder : mAH) {
            if (holder.isSearch()) {
                searchHolder = holder;
                break;
        ...
        if (searchHolder == null) {
            mHeader.setup(mAH, null, getCurrentPage(), tabsHidden);
        } else {
            mHeader.setup(mAH,
                    (SearchRecyclerView) searchHolder.mRecyclerView,
                    getCurrentPage(),
                    tabsHidden);
        }
    ...
    protected void updateSearchResultsVisibility() {
        ...
        if (mHeader.isSetUp()) {
            mHeader.setCurrentActive(getCurrentPage());
        }
    }
    ...
    private void applyAdapterSideAndBottomPaddings(DeviceProfile grid) {
        ...
        mTabsController.setPadding(grid.allAppsLeftRightPadding, bottomPadding);
    }
    ...
    protected boolean shouldShowTabs() {
        return mTabsController.getShouldShowTabs();
        //return mHasWorkApps;
    }
    ...
    public AlphabeticalAppsList<T> getSearchResultList() {
        AlphabeticalAppsList<?> searchList = null;
        for (ActivityAllAppsContainerView<?>.AdapterHolder holder : mAH) {
            if (holder.isSearch()) {
                searchList = holder.mAppsList;
            }
        }

        return (AlphabeticalAppsList<T>) searchList;
    }
    ...
    public int getSearchHolder() {
        for (int i = 0; i < mAH.size(); i++) {
            if (mAH.get(i).isSearch()) {
                return i;
            }
        }
        return 0;
