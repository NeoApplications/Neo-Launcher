...
package com.saggitt.omega.animation
...
@Keep
class OmegaAppTransitionManagerImpl(context: Context) :
        LauncherAppTransitionManagerImpl(context) {

    private val launcher = OmegaLauncher.getLauncher(context)
    ...
    override fun getLauncherContentAnimator(isAppOpening: Boolean,
        ...
        if (!launcher.isInState(ALL_APPS) && !launcher.isInState(OVERVIEW)) {
            background.blurAlphas.getProperty(OmegaBackgroundView.ALPHA_INDEX_TRANSITIONS).value = blurAlphas[0]
            val blurAlpha = ObjectAnimator.ofFloat(
                    background.blurAlphas.getProperty(OmegaBackgroundView.ALPHA_INDEX_TRANSITIONS),
    ...
    override fun createLauncherResumeAnimation(anim: AnimatorSet) {
        ...
        background.blurAlphas.getProperty(OmegaBackgroundView.ALPHA_INDEX_TRANSITIONS).value = 1f
    ...
    override fun resetContentView() {
        ...
        background.blurAlphas.getProperty(OmegaBackgroundView.ALPHA_INDEX_TRANSITIONS).value = 0f
    }
}
