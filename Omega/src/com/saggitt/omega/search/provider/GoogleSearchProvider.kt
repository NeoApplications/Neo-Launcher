...
package com.saggitt.omega.search.provider
...
import com.saggitt.omega.search.GOOGLE_QSB
...
class GoogleSearchProvider(context: Context) : SearchProvider(context) {
    ...
    override val isAvailable: Boolean
        get() = context.packageManager.isAppEnabled(GOOGLE_QSB, 0)
    ...
    override fun startSearch(callback: (intent: Intent) -> Unit) =
        callback(Intent().setClassName(GOOGLE_QSB, "$GOOGLE_QSB.SearchActivity"))
    ...
    override fun startAssistant(callback: (intent: Intent) -> Unit) =
        callback(
            Intent(Intent.ACTION_VOICE_COMMAND).addFlags(268468224).setPackage(GOOGLE_QSB)
        )
