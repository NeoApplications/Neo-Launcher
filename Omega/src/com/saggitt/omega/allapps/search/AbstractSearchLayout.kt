...
abstract class AbstractSearchLayout(context: Context, attrs: AttributeSet? = null) :
    LinearLayout(context, attrs), SearchProviderController.OnProviderChangeListener {
        ...
        searchLogoView = findViewById<AppCompatImageView?>(R.id.search_engine_logo).apply {
            ...
            setOnClickListener {
                if (searchProvider.supportsFeed) {
                    ...
                    controller.searchProvider.startSearch(mContext) { intent: Intent? ->
    ...
    */
    ...
    fun reloadPreferences(sharedPreferences: SharedPreferences) {
        post {
            ...
            //val showMic = sharedPreferences.getBoolean(PREFS_SEARCH_SHOW_ASSISTANT, true) && providerSupported
    ...
    protected open fun getCornerRadius(): Float {
        ...
        val radius: Float = round(Utilities.getOmegaPrefs(context).searchBarRadius.getValue())
        if (radius >= 0f) {
            return radius
        }
    ...
    open fun getLauncher(): NeoLauncher {
        return mActivity as NeoLauncher
    }
}