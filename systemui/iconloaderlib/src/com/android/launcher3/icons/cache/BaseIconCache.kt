...
constructor(
    ...
    class CacheEntry {
        @JvmField
        var bitmap: BitmapInfo = BitmapInfo.LOW_RES_INFO
        @JvmField
    ...
    @JvmField
    val workerHandler = Handler(bgLooper)

    @JvmField
    protected var iconDb = IconDB(context, dbFileName, iconPixelSize)
        ...
        if (entry == null || entry.bitmap.matchingLookupFlag.isVisuallyLessThan(lookupFlags)) {
            ...
            if (!getEntryFromDBLocked(cacheKey, entry, lookupFlags, appInfoCachingLogic)) {
                try {
                    ...
                    // package updates.
                    appInfoCachingLogic.getFreshnessIdentifier(appInfo, iconProvider)
                        ?.let { freshnessId ->
    ...
    companion object {
        ...
        @JvmField
        val RELEASE_VERSION = if (Flags.forceMonochromeAppIcons()) 10 else 9

        @JvmField
        val TABLE_NAME = "icons"
        @JvmField
        val COLUMN_ROWID = "rowid"
        @JvmField
        val COLUMN_COMPONENT = "componentName"
        @JvmField
        val COLUMN_USER = "profileId"
        @JvmField
        val COLUMN_FRESHNESS_ID = "freshnessId"
        @JvmField
        val COLUMN_ICON = "icon"
        @JvmField
        val COLUMN_ICON_COLOR = "icon_color"
        @JvmField
        val COLUMN_MONO_ICON = "mono_icon"
        @JvmField
        ...
        @JvmField
        val INDEX_TITLE = COLUMNS_HIGH_RES.indexOf(COLUMN_LABEL)
        @JvmField
        val INDEX_COLOR = COLUMNS_HIGH_RES.indexOf(COLUMN_ICON_COLOR)
        @JvmField
        val INDEX_FLAGS = COLUMNS_HIGH_RES.indexOf(COLUMN_FLAGS)
        @JvmField
        val INDEX_ICON = COLUMNS_HIGH_RES.indexOf(COLUMN_ICON)
        @JvmField
        val INDEX_MONO_ICON = COLUMNS_HIGH_RES.indexOf(COLUMN_MONO_ICON)
        @JvmField
        val INDEX_FRESHNESS_ID = COLUMNS_HIGH_RES.indexOf(COLUMN_FRESHNESS_ID)
